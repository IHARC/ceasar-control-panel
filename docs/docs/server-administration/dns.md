...
## Host DNS for your domain on Ceasar

Pre-requisites

These steps require that you configure the DNS servers of your domain to use your Ceasar servers.
...
1. On your Ceasar master, [create a DNS Zone](../user-guide/dns#adding-a-dns-zone) with the **child-ns** template
2. On your domain registrar panel, set the name servers of the domain to the Ceasar servers
...
A Master server is where DNS zones are created, and a Slave server receives the zone via the API. Ceasar can be configured as Master <-> Master or Master -> Slave. With a Master <-> Master configuration, each Master is also a Slave, so it could be considered as Master/Slave <-> Master/Slave.
...
### Master <-> Master DNS cluster (Default setup) with the Ceasar API

::: warning
This method does not support DNSSEC!
:::

1. Create a new user on each Ceasar server that will act as a “Slave”. Make sure it uses the username of "dns-cluster" or has the role `dns-cluster`
...
### Master -> Slave DNS cluster with the Ceasar API

::: info
It doesn't work if you try to sync via local network! See [Issue](https://github.com/iharc-jordan/ceasar-control-panel/issues/4295) Make sure to use the public ip addresses
:::
...
5. Edit `/usr/local/ceasar/conf/ceasar.conf`, change `DNS_CLUSTER_SYSTEM='ceasar'` to `DNS_CLUSTER_SYSTEM='ceasar-zone'`.
...
1. On the **Master** server, open `/usr/local/ceasar/conf/ceasar.conf`, change `DNS_CLUSTER_SYSTEM='ceasar'` to `DNS_CLUSTER_SYSTEM='ceasar-zone'`.
...
1. On **Master** and **Slave** servers, open `/usr/local/ceasar/conf/ceasar.conf`, change `DNS_CLUSTER_SYSTEM='ceasar'` to `DNS_CLUSTER_SYSTEM='ceasar-zone'`.
...
::: warning
DNSSEC can’t be used when Ceasar Cluster is active as Master <-> Master
:::
...
::: danger
Removing or disabling the private key in Ceasar will make the domain inaccessible.
:::
...
```bash
/usr/local/ceasar/func/remote.sh: line 43: return: Error:: numeric argument required
