...
2. On your Ceasar server run the following command (`mysql` may be replaced by `postgresql`):
...
Make sure the API is enabled and working properly. Ceasar’s PhpMyAdmin Single Sign On function connects over the Ceasar API.
...
```bash
v-add-database-host mysql db.ceasar.com root mypassword 500
```
...
See `/usr/local/ceasar/install/deb/phpmyadmin/pma.sh`
