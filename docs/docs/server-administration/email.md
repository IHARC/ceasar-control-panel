...
By default, emails generated by Ceasar (Notifications, Forgot password, update logs, etc.) are sent the internal mail. If you want, you can setup mail to be send over a SMTP account.

Execute the following script and follow the instructions:

```bash
bash /usr/local/ceasar/install/upgrade/manual/configure-server-smtp.sh
```
...
If you prefer to run cleanup from cron, create a script such as `/usr/local/sbin/ceasar-purge-mail-folders`:
...
"$CEASAR/bin/v-list-users" list | while read -r user; do
	while read -r domain; do
		...
		done < <("$CEASAR/bin/v-list-mail-accounts" "$user" "$domain" plain 2> /dev/null | cut -f1 || true)
	done < <("$CEASAR/bin/v-list-mail-domains" "$user" plain 2> /dev/null | cut -f1 || true)
done
```
...
```bash
chmod 750 /usr/local/sbin/ceasar-purge-mail-folders
```

Run it manually first. By default, the script only prints what it would purge:

```bash
/usr/local/sbin/ceasar-purge-mail-folders
```

Run the cleanup by setting `DRY_RUN=no`:

```bash
DRY_RUN=no /usr/local/sbin/ceasar-purge-mail-folders
```

Then add a root cron job, for example to run daily at 03:30:

```text
30 3 * * * DRY_RUN=no /usr/local/sbin/ceasar-purge-mail-folders >/var/log/ceasar-purge-mail-folders.log 2>&1
```
...
1. Run `v-add-sys-mail-dnsbl HereYourQueryKey.zen.dq.spamhaus.net` to add your custom DQS key. (Note: Ceasar automatically prevents your Spamhaus Query key from leaking in bounce messages).
...
No, Cloudflare’s Proxy does not work with email. If you use email hosted on you server, make sure the proxy for the A record `mail.domain.tld` is switched off. Otherwise, you will not be able receive emails. The following records are suggested if you want to use Ceasar as your mail server:
...
During Ceasar’s installation, use the `--sieve` flag. If Ceasar is already installed, there is a an upgrade script provided at the following path: `/usr/local/ceasar/install/upgrade/manual/install_sieve.sh`
...
[See forum topic for more info](https://forum.ceasar.com/t/oracle-cloud-email-as-relay-doesnt-works/11304/19?)
...
Create: $CEASAR/data/extensions/add-mail-domain.sh and $CEASAR/data/extensions/v-delete-mail-domain.sh
