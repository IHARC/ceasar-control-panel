...
::: warning
After every edit or update the firewall, Ceasar will clear the current iptables unless the rules are added via Ceasar and [custom script](#how-can-i-customize-iptables-rules).
:::
...
6. Select the data source by entering one of the following:
   ...
   - Script (with `chmod 755`): `/usr/local/ceasar/install/deb/firewall/ipset/blacklist.sh`
   - File: `file:/location/of/file`
   - You can also use one of Ceasar’s included sources.
...
Ceasar supports setting custom rules, chains or flags, etc. using script.

Script must be here: `/usr/local/ceasar/data/firewall/custom.sh`

1. Create custom.sh: `touch /usr/local/ceasar/data/firewall/custom.sh`
2. Make it executable: `chmod +x /usr/local/ceasar/data/firewall/custom.sh`
