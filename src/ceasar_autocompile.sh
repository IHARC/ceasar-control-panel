...
usage() {
	...
	echo "                    (e.g. --pkgrev 2 → 1.0.11-2+ubuntu24.04)."
	echo "    --release <id>  Set a release identifier appended to the package version"
	echo "                    as '~<id>' (e.g. --release myci1 → 1.0.11-1+ubuntu24.04~myci1)."
	...
	echo "                    Can be combined: --pkgrev 2 --release myci1 → 1.0.11-2+ubuntu24.04~myci1."
	echo ""
	...
	echo ""
	echo "Ceasar 1.0.11 builds only on Ubuntu 24.04 amd64."
}
...
for i in $*; do
	case "$i" in
		...
		--keepbuild)
			...
			;;
		--cross)
			echo "Cross builds are not supported. Ceasar 1.0.11 targets Ubuntu 24.04 amd64." >&2
			exit 1
			;;
		--help | -h)
			usage
...
if [ "$platform_id" != 'ubuntu' ] || [ "$platform_release" != '24.04' ]; then
	echo "Ceasar 1.0.11 packages must be built on Ubuntu 24.04." >&2
	exit 1
fi
if [ "$architecture" != 'x86_64' ] && [ "$architecture" != 'amd64' ]; then
	echo "Ceasar 1.0.11 packages must be built for amd64." >&2
	exit 1
fi
