...
usage() {
	...
	echo "                    (e.g. --pkgrev 2 → 1.0.27-2+ubuntu24.04)."
	echo "    --release <id>  Set a release identifier appended to the package version"
	echo "                    as '~<id>' (e.g. --release myci1 → 1.0.27-1+ubuntu24.04~myci1)."
	...
	echo "                    Can be combined: --pkgrev 2 --release myci1 → 1.0.27-2+ubuntu24.04~myci1."
	echo ""
	...
	echo ""
	echo "Ceasar 1.0.27 builds only on Ubuntu 24.04 amd64."
}
...
REPO='IHARC/ceasar-control-panel'
...
for i in $*; do
	case "$i" in
		...
		--keepbuild)
			...
			;;
		--cross)
			echo "Cross builds are not supported. Ceasar 1.0.27 targets Ubuntu 24.04 amd64." >&2
			exit 1
			;;
		--help | -h)
			usage
...
if [ "$platform_id" != 'ubuntu' ] || [ "$platform_release" != '24.04' ]; then
	echo "Ceasar 1.0.27 packages must be built on Ubuntu 24.04." >&2
	exit 1
fi
if [ "$architecture" != 'x86_64' ] && [ "$architecture" != 'amd64' ]; then
	echo "Ceasar 1.0.27 packages must be built for amd64." >&2
	exit 1
fi
...
CEASAR_ARCHIVE_LINK='https://github.com/IHARC/ceasar-control-panel/archive/'$branch'.tar.gz'
if [[ $NGINX_V =~ - ]]; then
...
if [ "$CEASAR_B" = true ]; then
	...
	for BUILD_ARCH in $arch; do
		for os in $supported_os; do
			...
			mkdir -p "$BUILD_DIR_CEASAR/etc/systemd/system"
			get_branch_file 'src/deb/ceasar/ceasar-support.service' "$BUILD_DIR_CEASAR/etc/systemd/system/ceasar-support.service"
			get_branch_file 'src/deb/ceasar/ceasar-support.timer' "$BUILD_DIR_CEASAR/etc/systemd/system/ceasar-support.timer"
