...
/* Ceasar way to enable support for SSO to PHPmyAdmin */
...
define("API_CEASAR_PORT", "%API_CEASAR_PORT%");
...
class Ceasar_API {
	...
	public function __construct() {
		$this->hostname = "https://" . API_HOST_NAME . ":" . API_CEASAR_PORT . "/api/";
		$this->key = API_KEY;
...
if (!empty($_GET)) {
	if (isset($_GET["logout"])) {
		$api->delete_temp_user(
			$_SESSION["CEASAR_sso_database"],
			$_SESSION["CEASAR_sso_user"],
			$_SESSION["PMA_single_signon_user"],
			$_SESSION["CEASAR_sso_host"],
		);
		//remove session
		session_invalid();
	} else {
		if (isset($_GET["user"]) && isset($_GET["ceasar_token"])) {
			$database = $_GET["database"];
			$user = $_GET["user"];
			$host = "localhost";
			$token = $_GET["ceasar_token"];
			...
			if ($time + 60 > time()) {
				...
				if ($data) {
					...
					$_SESSION["CEASAR_sso_user"] = $user;
					$_SESSION["CEASAR_sso_database"] = $database;
					$_SESSION["CEASAR_sso_host"] = $host;
