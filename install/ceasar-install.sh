...
# Final package version, for example 1.0.0-1+ubuntu24.04.
...
validate_platform() {
	...
	if [ "$platform_id" != 'ubuntu' ] || [ "$release" != '24.04' ]; then
		check_result 1 "Ceasar 1.0.0 supports only Ubuntu 24.04 LTS."
	fi
	if [ "$architecture" != 'x86_64' ] && [ "$architecture" != 'amd64' ]; then
		check_result 1 "Ceasar 1.0.0 supports only amd64 systems."
	fi
	if [ -n "$codename" ] && [ "$codename" != 'noble' ]; then
		check_result 1 "Ceasar 1.0.0 requires the Ubuntu noble package repositories."
	fi
	codename='noble'
}
...
if [ -n "$withdebs" ]; then
	...
	echo "    - ceasar core package"
	dpkg -i "$withdebs"/ceasar_*.deb > /dev/null 2>&1
	echo "    - ceasar-php backend package"
	dpkg -i "$withdebs"/ceasar-php_*.deb > /dev/null 2>&1
	echo "    - ceasar-nginx backend package"
	dpkg -i "$withdebs"/ceasar-nginx_*.deb > /dev/null 2>&1

	if [ "$webterminal" = "yes" ]; then
		echo "    - ceasar-web-terminal"
		dpkg -i "$withdebs"/ceasar-web-terminal_*.deb > /dev/null 2>&1
	fi
fi
