...
# Final package version, for example 1.0.25-1+ubuntu24.04.
...
validate_platform() {
	...
	if [ "$platform_id" != 'ubuntu' ] || [ "$release" != '24.04' ]; then
		check_result 1 "Ceasar 1.0.25 supports only Ubuntu 24.04 LTS."
	fi
	if [ "$architecture" != 'x86_64' ] && [ "$architecture" != 'amd64' ]; then
		check_result 1 "Ceasar 1.0.25 supports only amd64 systems."
	fi
	if [ -n "$codename" ] && [ "$codename" != 'noble' ]; then
		check_result 1 "Ceasar 1.0.25 requires the Ubuntu noble package repositories."
	fi
	codename='noble'
}
...
if customer_enabled:
    required = {
        "schema", "enabled", "supabase_url", "supabase_publishable_key",
    ...
    if not required.issubset(customer) or customer.get("schema") != 1:
    ...
    customer["password_min_length"] = minimum
...
# Persistent support and branding state must survive later package upgrades.
$CEASAR/bin/v-setup-support-runtime
