...
# Final package version, for example 1.0.32-1+ubuntu24.04.
...
validate_platform() {
	...
	if [ "$platform_id" != 'ubuntu' ] || [ "$release" != '24.04' ]; then
		check_result 1 "Ceasar 1.0.32 supports only Ubuntu 24.04 LTS."
	fi
	if [ "$architecture" != 'x86_64' ] && [ "$architecture" != 'amd64' ]; then
		check_result 1 "Ceasar 1.0.32 supports only amd64 systems."
	fi
	if [ -n "$codename" ] && [ "$codename" != 'noble' ]; then
		check_result 1 "Ceasar 1.0.32 requires the Ubuntu noble package repositories."
	fi
	codename='noble'
}
