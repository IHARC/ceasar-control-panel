...
@test "[Fail][ APIV2 ] Create new user" {
    run curl -k -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "hash=$accesskey&returncode=yes&cmd=v-add-user&arg1=ceasartest&arg2=strongpassword&arg3=info@ceasar.com" "https://$server:$port/api/index.php"
    assert_success
...
@test "[Success][ Hash ] Create new user" {
    run curl -k -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "hash=$apikey&cmd=v-add-user&arg1=ceasartest&arg2=/tmp/clusterpassword&arg3=info@ceasar.com&arg4=default" "https://$server:$port/api/index.php"
    assert_success
    assert_output --partial "OK"
}

@test "[Success][ Hash ] Check password" {
    run curl -k -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "hash=$apikey&cmd=v-check-user-password&arg1=ceasartest&arg2=strongpassword" "https://$server:$port/api/index.php"
    assert_success
    assert_output --partial "OK"
}
...
@test "[Success][ Local ] Add user" {
    run v-add-user ceasartest 1234BCD info@ceasar.com
    assert_success
}

@test "[Success][ Local ] Add DNS domain" {
    run v-add-dns-domain ceasartest iloveceasar.com 127.0.0.1
    assert_success
}

@test "[Success][ APIV2 ] Add remote DNS host" {
    run v-add-remote-dns-host $server $port "$accesskey" '' api 'ceasartest'
    assert_success
}
...
@test "[Success][ Local ] nslookup iloveceasar.com" {
    run nslookup iloveceasar.com $server
    assert_success
    assert_output --partial "127.0.0.1"
}

@test "[Success][ Local ] Add DNS domain 2" {
    run v-add-dns-domain ceasartest iloveceasar.org 127.0.0.1
    assert_success
}

@test "[Success][ Local ] Add DNS record" {
    run v-add-dns-record ceasartest iloveceasar.org test A 127.0.0.1 yes 20
    assert_success
}

@test "[Success][ Local ] nslookup test.iloveceasar.org" {
    run nslookup test.iloveceasar.org $server
    assert_failure 1
    assert_output --partial "REFUSED"

    run nslookup test.iloveceasar.org localhost
    assert_success
    assert_output --partial "127.0.0.1"
}
...
@test "[Success][ Local ] Delete DNS record" {
    run v-delete-dns-record ceasartest iloveceasar.org 20
    assert_success
}

@test "[Success][ Local ] nslookup test.iloveceasar.org 2" {
    run nslookup test.iloveceasar.org $server
    assert_success
    assert_output --partial "127.0.0.1"
...
@test "[Success][ Local ] Delete user" {
    run v-delete-user ceasartest
    assert_success
}

@test "[Success][ Hash ] Delete user" {
    run curl -k -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "hash=$apikey&cmd=v-delete-user&arg1=ceasartest" "https://$server:$port/api/index.php"
}
