...
if ($_SESSION["userContext"] === "admin" && $_SESSION["look"] == "") {
	if (empty($_GET["user"])) {
		exec(CEASAR_CMD . "v-list-users-stats json", $output, $return_var);
		$data = json_decode(implode("", $output), true);
		$data = array_reverse($data, true);
		unset($output);
	} else {
		$v_user = quoteshellarg($_GET["user"]);
		exec(CEASAR_CMD . "v-list-user-stats $v_user json", $output, $return_var);
		$data = json_decode(implode("", $output), true);
		$data = array_reverse($data, true);
		unset($output);
	}
	...
	exec(CEASAR_CMD . "v-list-user-stats $user json", $output, $return_var);
	$data = json_decode(implode("", $output), true);
	$data = array_reverse($data, true);
	unset($output);
}
