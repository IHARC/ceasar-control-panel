...
if (!empty($_POST["user"]) && empty($_POST["code"])) {
	...
	$user = $_POST["user"];
	$email = $_POST["email"];
	$cmd = "/usr/bin/sudo /usr/local/ceasar/bin/v-list-user";
	exec($cmd . " " . $v_user . " json", $output, $return_var);
	if ($return_var == 0) {
		$data = json_decode(implode("", $output), true);
		unset($output);
		exec(CEASAR_CMD . "v-get-user-value " . $v_user . " RKEYEXP", $output, $return_var);
		...
		if ($rkeyexp === null || $rkeyexp < time() - 1) {
			if ($email == $data[$user]["CONTACT"]) {
				...
				fclose($fp);
				exec(
					CEASAR_CMD . "v-change-user-rkey " . $v_user . " " . $v_rkey . "",
					$output,
					$return_var,
				);
				unset($output);
		...
		if ($return_var == 0) {
			...
			if (password_verify($_POST["code"], $rkey)) {
				...
				if ($output[0] > time() - 900) {
					...
					fclose($fp);
					exec(
						CEASAR_CMD . "v-change-user-password " . $v_user . " " . $v_password,
						$output,
						$return_var,
					);
					unlink($v_password);
