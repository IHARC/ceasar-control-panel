...
function customer_bootstrap(): void
{
    ...
    $connectSource = parse_url((string) $config['supabase_url'], PHP_URL_SCHEME)
        . '://'
...
function customer_request_origin(): string
{
    ...
    if (!is_string($host) || preg_match('/^(?:[A-Za-z0-9.-]+|\[[0-9A-Fa-f:]+\])(?::[0-9]{1,5})?$/D', $host) !== 1) {
...
function customer_require_rp_id(string $rpId, bool $fixture): void
{
    ...
    if (
        ...
        throw new RuntimeException('Passkey relying-party ID does not match the customer hostname.');
    }
}
