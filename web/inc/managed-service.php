...
function managed_service_require_authorized_session(): void {
	if (!managed_service_authorized_session()) {
		throw new RuntimeException(
			_("Managed services require a direct administrator session. Sign in again."),
		);
	}
}
...
function managed_service_idempotency_key(array $input): string {
	...
	if ($key === null) {
		throw new RuntimeException(
			_("Refresh the page before submitting this managed-service request."),
		);
	}
	return $key;
}
...
function managed_service_form_request(array $input): array {
	...
	if (!is_string($operation) || $section === null) {
	...
	if ($operation === "set_trial_capacity") {
		...
		return [
			...
			"payload" => ["concurrent_limit" => (int) $limit],
		];
	}
...
function managed_service_read_request(array $input): array {
	...
	if ($section === null) {
		throw new RuntimeException(_("The requested managed-service view is unavailable."));
	}
	$request = ["operation" => "read", "section" => $section];
	if (array_key_exists("record_id", $input)) {
		$recordId = managed_service_uuid($input["record_id"]);
		if ($recordId === null) {
			throw new RuntimeException(_("The requested managed-service record is unavailable."));
		}
...
function managed_service_request(array $request): array {
	...
	if (strlen($payload) > 32768) {
		throw new RuntimeException(_("Managed-service request is too large."));
	}
	$pipes = [];
	$process = proc_open(
		["/usr/bin/sudo", CEASAR_DIR_BIN . "v-managed-service", $_SESSION["user"]],
		[["pipe", "r"], ["pipe", "w"], ["pipe", "w"]],
		$pipes,
	);
	if (!is_resource($process)) {
		throw new RuntimeException(_("Managed-service adapter is unavailable."));
	}
	...
	if (proc_close($process) !== 0) {
		throw new RuntimeException(_("Managed-service request failed."));
	}
	try {
