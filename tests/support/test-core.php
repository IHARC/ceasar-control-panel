...
$db = support_db();
$htmlOnlyBody = support_inbound_body(
	"",
	"<html><body><p>Hello&nbsp;Jordan</p><div>Second line &amp; detail</div></body></html>",
);
assert_true(
	$htmlOnlyBody === "Hello Jordan\nSecond line & detail",
	"HTML-only inbound email is stored as readable text",
);
$headOnlyBody = support_inbound_body(
	"",
	"<html><head><style>p { margin: 0 }</style><script>alert(1)</script><title>Ignored title</title></head><body><p>Readable reply</p></body></html>",
);
assert_true(
...
$smtp = support_normalize_smtp(
	...
	["SERVER_SMTP_PASSWD" => "existing-secret"],
);
assert_true(
	$smtp["password"] === "existing-secret" && $smtp["security"] === "tls",
	"SMTP settings preserve a blank password and normalize security",
);
assert_true(
	(support_smtp_public([
		"USE_SERVER_SMTP" => "true",
		"SERVER_SMTP_ADDR" => "mailer@example.test",
		"SERVER_SMTP_PASSWD" => "secret",
	])["passwordConfigured"] ??
		false) ===
		true,
...
assert_true(
	file($smtpArgsFile, FILE_IGNORE_NEW_LINES) === [
		"smtp.example.test",
		"587",
		"tls",
		"mailer@example.test",
	...
	"SMTP command receives each validated argument unchanged",
);
...
$uncertainCalls = 0;
$GLOBALS["ceasar_support_mailer_for_test"] = function () use (&$uncertainCalls): array {
	$uncertainCalls++;
	return ["accepted" => false, "uncertain" => true, "error" => "DATA connection dropped"];
};
support_deliver_outbox(1);
support_deliver_outbox(1);
assert_true(
	$db->query("SELECT state FROM support_outbox WHERE id='smtp-uncertain'")->fetchColumn() ===
		"uncertain" && $uncertainCalls === 1,
	"worker never automatically resends an uncertain SMTP outcome",
);
