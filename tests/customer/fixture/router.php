...
if (is_string($path)) {
    ...
    if ($static !== false && is_file($static) && str_starts_with($static, realpath($documentRoot))) {
        return false;
    }
}
...
{
    $header = fixture_base64url(json_encode(['alg' => 'none', 'typ' => 'JWT'], JSON_THROW_ON_ERROR));
    $payload = fixture_base64url(json_encode([
