...
#===========================================================================#
#                                                                           #
# Ceasar Control Panel - Upgrade Function Library                           #
#                                                                           #
#===========================================================================#
...
add_upgrade_message() {
	if [ -f "$CEASAR_BACKUP/message.log" ]; then
		echo -e $1 >> $CEASAR_BACKUP/message.log
		echo -e "\n\n" >> $CEASAR_BACKUP/message.log
	else
		echo -e $1 > $CEASAR_BACKUP/message.log
	fi
}
...
upgrade_health_check() {
	...
	# Perform basic health check against ceasar.conf to ensure that
	...
	if [ -z "$VERSION" ]; then
		...
		echo
		echo "[ ! ] Unable to detect installed version of Ceasar Control Panel."
...
upgrade_welcome_message() {
	...
	echo "                                                                              "
	echo "                    Ceasar Control Panel Software Update                     "
	...
	echo ""
	echo "Backup directory: $CEASAR_BACKUP/"
	echo "Installation log: $LOG"
}

upgrade_welcome_message_log() {
	echo "============================================================================="
	echo "Ceasar Control Panel Software Update Log"
	echo "============================================================================="
	echo
...
upgrade_complete_message() {
	...
	echo "The server-owned Ceasar Control Panel configuration remains in place.     "
	echo "Review the panel's White Label settings if the customer-facing identity   "
	echo "needs to be changed.                                                       "
	echo
	echo "Read the release notes included with the reviewed source baseline.         "
	echo
	echo "Thank you for using Ceasar Control Panel.                                  "
	echo
	echo "============================================================================="
	echo
}
...
upgrade_get_version() {
	# Retrieve new version number for Ceasar Control Panel from .deb package
	new_version=$(dpkg -l | awk '$2=="ceasar" { print $3 }')
}

upgrade_set_version() {
	# Set new version number in ceasar.conf
	$BIN/v-change-sys-config-value "VERSION" "$@"
}

upgrade_set_branch() {
	# Set branch in ceasar.conf
...
upgrade_send_notification_to_panel() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		...
		$BIN/v-add-user-notification "$ROOT_USER" 'Development snapshot installed' '<p><span class="u-text-bold">Version:</span> '$new_version'<br><span class="u-text-bold">Code Branch:</span> '$RELEASE_BRANCH'</p><p><i class="fas fa-heart icon-red"></i> The Ceasar Control Panel development team</p>'
	elif [[ "$new_version" =~ "beta" ]]; then
		# Send feedback notification for beta releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Ceasar Control Panel test build '$new_version'.' '<p>Test build installed. Review the release notes included with the reviewed source baseline.</p><p><i class="fas fa-heart icon-red"></i> The Ceasar Control Panel development team</p>'
	else
		# Send normal upgrade complete notification for stable releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Upgrade complete' '<p>Ceasar Control Panel has been updated to <span class="u-text-bold">v'$new_version'</span>.</p><p class="u-text-bold">Have a wonderful day!</p><p><i class="fas fa-heart icon-red"></i> The Ceasar Control Panel development team</p>'
	fi
}
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		message_tmp_file="/tmp/ceasar-upgrade-complete.txt"

		# Create temporary file
		touch $message_tmp_file

		# Write message to file
		echo "$HOSTNAME has been upgraded from Ceasar Control Panel v$VERSION to v${new_version}." >> $message_tmp_file
		...
		if [[ -f "$CEASAR_BACKUP/message.log" ]]; then
			...
			echo "" >> $message_tmp_file
			cat $CEASAR_BACKUP/message.log >> $message_tmp_file
			echo "" >> $message_tmp_file
			echo "===================================================" >> $message_tmp_file
			echo "" >> $message_tmp_file
		fi
...
upgrade_config_set_value() {
	if [ -f "$CEASAR_BACKUP/upgrade.conf" ]; then
		if [ "$2" = "true" ]; then
			sed -i "s/$1='false'/$1='true'/g" $CEASAR_BACKUP/upgrade.conf
		fi
	fi
}

prepare_upgrade_config() {
	mkdir -p $CEASAR_BACKUP
	touch $CEASAR_BACKUP/upgrade.conf
	while IFS='= ' read -r lhs rhs; do
		if [[ ! $lhs =~ ^\ *# && -n $lhs ]]; then
			...
			echo "$lhs='$rhs'" >> $CEASAR_BACKUP/upgrade.conf
		fi
	done < "$CEASAR/install/upgrade/upgrade.conf"
}

upgrade_init_backup() {
	# Ensure that backup directories are created
	# Ceasar Control Panel configuration files
	mkdir -p $CEASAR_BACKUP/conf/ceasar/

	# OpenSSL configuration files
	mkdir -p $CEASAR_BACKUP/conf/openssl/

	# Hosting Packages
	mkdir -p $CEASAR_BACKUP/packages/

	# Domain template files
	mkdir -p $CEASAR_BACKUP/templates/

	# System services (apache2, nginx, bind9, vsftpd, etc).
	if [ -n "$WEB_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$DNS_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			mkdir -p $CEASAR_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			mkdir -p $CEASAR_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$FTP_SYSTEM/
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		mkdir -p $CEASAR_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		mkdir -p $CEASAR_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube/" ]; then
		mkdir -p $CEASAR_BACKUP/conf/roundcube/
	fi
	if [ -d "/var/lib/snappymail/" ]; then
		mkdir -p $CEASAR_BACKUP/conf/snappymail/
	fi
	if [ -d "/etc/phpmyadmin/" ]; then
		mkdir -p $CEASAR_BACKUP/conf/phpmyadmin/
	fi
	if [ -d "/etc/phppgadmin/" ]; then
		mkdir -p $CEASAR_BACKUP/conf/phppgadmin/
	fi
}

upgrade_init_logging() {
	# Set log file path
	LOG="$CEASAR_BACKUP/ceasar-upgrade-$(date +%d%m%Y%H%M).log"
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Review the server upgrade log for details."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Review the server upgrade log for details."
	fi
}
...
upgrade_start_backup() {
	...
	cp -fr $CEASAR/data/packages/* $CEASAR_BACKUP/packages/

	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - Templates"
	fi
	cp -fr $CEASAR/data/templates/* $CEASAR_BACKUP/templates/
	...
	cp -fr $CEASAR/conf/* $CEASAR_BACKUP/conf/ceasar/

	# OpenSSL configuration files
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      ---- openssl"
	fi
	cp -f /etc/ssl/*.cnf $CEASAR_BACKUP/conf/openssl/
	...
	if [ -n "$WEB_SYSTEM" ]; then
		...
		cp -fr /etc/$WEB_SYSTEM/* $CEASAR_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $PROXY_SYSTEM"
		fi
		cp -fr /etc/$PROXY_SYSTEM/* $CEASAR_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $IMAP_SYSTEM"
		fi
		cp -fr /etc/$IMAP_SYSTEM/* $CEASAR_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $MAIL_SYSTEM"
		fi
		cp -fr /etc/$MAIL_SYSTEM/* $CEASAR_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		if [ "$DNS_SYSTEM" = "bind9" ]; then
			...
			cp -fr /etc/bind/* $CEASAR_BACKUP/conf/$DNS_SYSTEM/
		fi
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			...
			cp -fr /etc/mysql/* $CEASAR_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			if [ "$DEBUG_MODE" = "true" ]; then
				echo "      ---- pgsql"
			...
			cp -fr /etc/postgresql/* $CEASAR_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		if [ "$FTP_SYSTEM" = "vsftpd" ]; then
			cp -f /etc/$FTP_SYSTEM.conf $CEASAR_BACKUP/conf/$FTP_SYSTEM/
		fi
		if [ "$FTP_SYSTEM" = "proftpd" ]; then
			cp -f /etc/proftpd/proftpd.conf $CEASAR_BACKUP/conf/$FTP_SYSTEM/
		fi
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		[ -e "/etc/sysconfig/iptables" ] && cp -f /etc/sysconfig/iptables $CEASAR_BACKUP/conf/$FIREWALL_SYSTEM/
		[ -e "/etc/iptables.rules" ] && cp -f /etc/iptables.rules $CEASAR_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $FIREWALL_EXTENSION"
		fi
		cp -f /etc/$FIREWALL_EXTENSION/*.conf $CEASAR_BACKUP/conf/$FIREWALL_EXTENSION/
		cp -f /etc/$FIREWALL_EXTENSION/*.local $CEASAR_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- sshd"
		fi
		cp -fr /etc/ssh/* $CEASAR_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- Roundcube"
		fi
		cp -fr /etc/roundcube/* $CEASAR_BACKUP/conf/roundcube
	fi
	if [ -d "/var/lib/snappymail" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- SnappyMail"
		fi
		cp -fr /var/lib/snappymail/* $CEASAR_BACKUP/conf/snappymail
	fi
	if [ -d "/etc/phpmyadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phpMyAdmin"
		fi
		cp -fr /etc/phpmyadmin/* $CEASAR_BACKUP/conf/phpmyadmin
	fi
	if [ -d "/etc/phppgadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phppgadmin"
		fi
		cp -fr /etc/phppgadmin/* $CEASAR_BACKUP/conf/phppgadmin
	fi
}

upgrade_refresh_config() {
	source_conf "/usr/local/ceasar/conf/ceasar.conf"
}
...
upgrade_start_routine() {
	...
	upgrade_steps=$(ls -v $CEASAR/install/upgrade/versions/*.sh)
	for script in $upgrade_steps; do
		declare -a available_versions
		available_versions+=($(echo $script | sed "s|/usr/local/ceasar/install/upgrade/versions/||g" | sed "s|.sh||g"))
	done
	...
	if [ $(check_version $latest_version) -gt $(check_version $VERSION) ]; then
		for version_step in "${available_versions[@]}"; do
			if [ $(check_version $VERSION) -lt $(check_version "$version_step") ]; then
				upgrade_step_message
				source $CEASAR/install/upgrade/versions/$version_step.sh
			fi
		done
		...
		echo ""
		echo "[ ! ] The latest version of Ceasar Control Panel is already installed."
		echo "      Verifying configuration..."
		echo ""
		if [ -e "$CEASAR/install/upgrade/versions/$VERSION.sh" ]; then
			source $CEASAR/install/upgrade/versions/$VERSION.sh
		fi
		VERSION="$new_version"
		upgrade_set_version "$VERSION"
		upgrade_refresh_config
...
upgrade_phppgadmin() {
	if [ -n "$(echo $DB_SYSTEM | grep -w 'pgsql')" ]; then
		...
		if version_ge "$pga_release" "$pga_v"; then
			...
			wget --retry-connrefused --quiet https://github.com/ceasar/phppgadmin/releases/download/v$pga_v/phppgadmin-v$pga_v.tar.gz
	...
	if [ -n "$(echo $DB_SYSTEM | grep -w 'mysql')" ]; then
		...
		if version_ge "$pma_version" "$pma_v"; then
			...
			if [ -e /var/lib/phpmyadmin/blowfish_secret.inc.php ]; then
				chown root:ceasarmail /var/lib/phpmyadmin/blowfish_secret.inc.php
				chmod 0640 /var/lib/phpmyadmin/blowfish_secret.inc.php
			fi
			chown ceasarmail:ceasarmail /usr/share/phpmyadmin/tmp
...
upgrade_filemanager() {
	FILE_MANAGER_CHECK=$(cat $CEASAR/conf/ceasar.conf | grep "FILE_MANAGER='false'")
	if [ -z "$FILE_MANAGER_CHECK" ]; then
		if [ -f "$CEASAR/web/fm/version" ]; then
			fm_version=$(cat $CEASAR/web/fm/version)
		else
			fm_version="1.0.0"
		fi
		if ! version_ge "$fm_version" "$fm_v"; then
			...
			if [ "$UPGRADE_UPDATE_FILEMANAGER_CONFIG" = "true" ]; then
				if [ -e "$CEASAR/web/fm/configuration.php" ]; then
					echo "[ ! ] Updating File Manager configuration..."
					# Update configuration.php
					cp -f $CEASAR_INSTALL_DIR/filemanager/filegator/configuration.php $CEASAR/web/fm/configuration.php

					# Path to the file manager configuration file where the change will be made.
					config_file="$CEASAR/web/fm/configuration.php"
...
upgrade_roundcube() {
	if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'roundcube')" ]; then
		if [ -d "/usr/share/roundcube" ]; then
			...
			echo "      To upgrade to the latest version of Roundcube directly from upstream, from please run the command migrate_roundcube.sh located in: /usr/local/ceasar/install/upgrade/manual/"
		else
...
upgrade_dependencies() {
	echo "[ ! ] Update Ceasar PHP dependencies..."
...
upgrade_restart_services() {
	if [ "$UPGRADE_RESTART_SERVICES" = "true" ]; then
		...
		if [ "$WEB_TERMINAL" = "true" ]; then
			if [ "$DEBUG_MODE" = "true" ]; then
				echo "      - ceasar-web-terminal"
			fi
			$BIN/v-restart-service "ceasar-web-terminal"
		fi
	...
	# Always restart the Ceasar Control Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - ceasar"
	fi
	$BIN/v-restart-service ceasar
}
